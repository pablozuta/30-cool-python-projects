...
class AddressBook:
    ...
    def initialization(self):
        main_frame = customtkinter.CTkFrame(self.parent)
        ...
        self.status_bar = Label(main_frame, text="code 2023", relief=SUNKEN, bd=1).pack(side=BOTTOM, fill=X)

        frame1 = customtkinter.CTkFrame(main_frame, border_width=2)
        ...
        frame3 = customtkinter.CTkFrame(frame2)
        frame3.pack(side=TOP, expand=YES)

        customtkinter.CTkLabel(frame3, text='Full Name').grid(row=0, column=0, sticky=W)
        self.ent_name = customtkinter.CTkEntry(frame3, textvariable=self.name_var, width=180)
        self.ent_name.grid(row=0, column=1)

        customtkinter.CTkLabel(frame3, text='Email').grid(row=1, column=0, sticky=W)
        self.ent_email = customtkinter.CTkEntry(frame3, textvariable=self.email_var, width=180)
        self.ent_email.grid(row=1, column=1)

        customtkinter.CTkLabel(frame3, text='Phone').grid(row=2, column=0, sticky=W)
        self.ent_phone = customtkinter.CTkEntry(frame3, textvariable=self.phone_var, width=180)
        self.ent_phone.grid(row=2, column=1)
        padding_inline = 5

        frame4 = customtkinter.CTkFrame(frame2)
        frame4.pack(side=BOTTOM, expand=YES)

        self.btn_new = customtkinter.CTkButton(frame4, text='New', command=self.on_new, width=25)
        self.btn_new.pack(side=LEFT, padx=padding_inline)
        self.btn_add = customtkinter.CTkButton(frame4, text='Add', command=self.on_add, width=15)
        self.btn_add.pack(side=LEFT, padx=padding_inline)
        self.btn_mod = customtkinter.CTkButton(frame4, text='Edit', command=self.on_mod, width=15)
        self.btn_mod.pack(side=LEFT, padx=padding_inline)
        self.btn_del = customtkinter.CTkButton(frame4, text='Delete', command=self.on_del, width=15)
        self.btn_del.pack(side=LEFT, padx=padding_inline)
...
if __name__ == '__main__':
    ...
    application = AddressBook(root, "BOOK OF CONTACTS")
